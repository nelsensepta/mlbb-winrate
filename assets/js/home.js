...
const wrReq = document.getElementById("WrReq");
...
function validation() {
  const resultNum = rumus(
    parseFloat(tMatch.value),
    parseFloat(tWr.value),
    parseFloat(wrReq.value)
  );
  const loseNum = rumusLose(
    parseFloat(tMatch.value),
    parseFloat(tWr.value),
    parseFloat(wrReq.value)
  );
  ...
  if (tMatch.value == "" || tWr.value == "" || wrReq.value == "") {
    ...
    parseFloat(tMatch.value) < 0 ||
    parseFloat(tWr.value) < 0 ||
    parseFloat(wrReq.value) < 0
  ) {
...
//   fetch(
//     "https://api.countapi.xyz/update/johsteven/ccac8049-f0da-408a-b6d7-8826b199afd0/?amount=1"
//   )
//     .then((res) => res.json())
//     .then((res) => {
